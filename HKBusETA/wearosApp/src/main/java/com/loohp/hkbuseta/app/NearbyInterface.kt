...
fun NoNearbyText(closestStop: Stop, distance: Double, instance: AppActiveContext) {
    ...
    Text(
        ...
        text = if (Shared.language == "en") {
            "Nearest Stop: ${closestStop.name.en} (${(distance * 1000).roundToInt().formatDecimalSeparator()}m)"
        } else {
            "最近的巴士站: ${closestStop.name.zh} (${(distance * 1000).roundToInt().formatDecimalSeparator()}米)"
        }
    )
}
...
fun MainElement(location: LocationResult?, exclude: ImmutableMap<Operator, Set<String>>, interchangeSearch: Boolean, instance: AppActiveContext) {
    ...
    LaunchedEffect (Unit) {
        ...
        if (locationResult?.isSuccess == true) {
            ...
            result = Registry.getInstance(instance).getNearbyRoutes(loc, exclude, interchangeSearch)
        }
        state = true
    }
    ...
    if (state) {
        if (result == null) {
            ...
            if (list.isEmpty()) {
                ...
                intent.putExtra("proximitySortOrigin", doubleArrayOf(result.origin.lat, result.origin.lng))
