...
class EtaTileServiceCommon {
    ...
    companion object {
        ...
        private fun etaText(eta: Registry.MergedETAQueryResult<Pair<FavouriteResolvedStop, FavouriteRouteStop>>?, seq: Int, mainResolvedStop: Pair<FavouriteResolvedStop, FavouriteRouteStop>, packageName: String, context: AppContext): LayoutElementBuilders.Spannable {
            val line = eta.getResolvedText(seq, Shared.etaDisplayMode, context)
            ...
            return LayoutElementBuilders.Spannable.Builder()
                ...
                .addContentAnnotatedString(text, textSize, { it.setColor(ColorProp.Builder(color).build()) }, { WearOSShared.RESOURCE_RATIO[it]!! * imageHeight to imageHeight }, { r -> addInlineImageResource(r) })
                .build()
        }
        ...
        private fun lastUpdated(context: AppContext): LayoutElementBuilders.Text {
            return LayoutElementBuilders.Text.Builder()
                ...
                .setText((if (Shared.language == "en") "Updated: " else "更新時間: ")
                    .plus(DateFormat.getTimeFormat(context.context).timeZone(TimeZone.getTimeZone(ZoneId.of(hongKongZoneId))).format(Date())))
                .setFontStyle(
                    FontStyle.Builder()
                        .setSize(
        ...
        private fun buildSuitableElement(tileId: Int, packageName: String, context: AppContext): LayoutElementBuilders.LayoutElement {
            ...
            return if (favouriteRoutes.isEmpty()) {
                noFavouriteRouteStop(tileId, packageName, context)
            } else {
                buildLayout(tileId, Shared.favoriteRouteStops.value.asSequence()
                    .flatMap { it.favouriteRouteStops }
                    .filter { favouriteRoutes.contains(it.favouriteId) }
                    .toList(), packageName, context)
            }
        }
        ...
        fun requestTileUpdate() {
            for ((_, tileState) in internalTileStates) {
                tileState.markShouldUpdate()
            }
        }
