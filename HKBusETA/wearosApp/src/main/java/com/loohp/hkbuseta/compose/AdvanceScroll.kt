...
val TransformingLazyColumnState.firstVisibleItemScrollOffset: Int get() = -(layoutInfo.visibleItems.firstOrNull()?.offset?: 0)
    ...
    drawWithContent {
        ...
        if (viewPortLength < contentLength) {
            ...
            if (context.isScreenRound()) {
                val topLeft = Offset(halfIndicatorThicknessPx + pad, halfIndicatorThicknessPx + pad)
                val size = Size(window.width - indicatorThicknessPx - pad * 2, window.height - indicatorThicknessPx - pad * 2)
                ...
                val topLeft = Offset(window.width - indicatorThicknessPx - pad, viewPortLength * 0.125F)
...
fun Modifier.fullPageVerticalScrollbar(
    ...
    drawWithContent {
        ...
        if (viewPortLength < contentLength) {
            ...
            if (context.isScreenRound()) {
                val topLeft = Offset(halfIndicatorThicknessPx + pad, halfIndicatorThicknessPx + pad)
                val size = Size(viewPortWidth - indicatorThicknessPx - pad * 2, viewPortLength - indicatorThicknessPx - pad * 2)
                val style = Stroke(width = indicatorThicknessPx, cap = StrokeCap.Round)
...
fun Modifier.fullPageVerticalScrollbar(
    ...
    var indicatorAlphaInit by remember { mutableStateOf(true) }
    var scrollValue by remember { mutableStateOf(state.anchorItemIndex to state.anchorItemScrollOffset) }
    var scrollValueChanged by remember { mutableStateOf(false) }
    val animatedIndicatorAlpha by animateFloatAsState(
        targetValue = if (scrollValueChanged || indicatorAlphaInit || (state.isScrollInProgress && (state.canScrollForward || state.canScrollBackward))) 1F else 0F,
        animationSpec = tween(
    ...
    val anchorItemScrollOffset by remember { derivedStateOf { state.anchorItemScrollOffset } }
    LaunchedEffect (anchorItemIndex, anchorItemScrollOffset) {
        val pair = anchorItemIndex to anchorItemScrollOffset
        if (scrollValue != pair) {
            scrollValueChanged = true
            scrollValue = pair
            delay(50)
    ...
    drawWithContent {
        ...
        val (viewPortWidth, viewPortLength) = size
        val itemsVisible = state.layoutInfo.visibleItems
        if (totalItemCount != state.layoutInfo.totalItemsCount) {
            actualItemLength.clear()
            totalItemCount = state.layoutInfo.totalItemsCount
        }

        itemsVisible.forEach { actualItemLength[it.index] = it.measuredHeight }
        val knownLength = actualItemLength.entries.sumOf { it.value }
        val knownAmount = actualItemLength.values.count()
        val knownAverageItemLength = knownLength / knownAmount
        val contentOffset = (0 until state.firstVisibleItemIndex).sumOf { actualItemLength.getOrDefault(it, knownAverageItemLength) }.toFloat() + state.firstVisibleItemScrollOffset
...
fun Modifier.transforming(
    scope: TransformingLazyColumnItemScope,
    transformationSpec: TransformationSpec,
): Modifier = this
    .transformedHeight(scope, transformationSpec)
    .graphicsLayer {
        scope.run { transformationSpec.run { applyContainerTransformation(scrollProgress) } }
    }
