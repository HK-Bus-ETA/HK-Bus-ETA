...
func favouriteRouteGroupEntries(_ name: String) -> [FavouriteRoute]? {
    ...
    return entries.filter { $0.contains { $0.key == "platformDisplayInfo" } }.map {
        ...
        let coDisplay: [ColorInfoString] = (display["coDisplay-\(language)"] as! [[String: Any]]).map {
            let string = $0["string"] as! String
            let color = $0["color"] as? Int64
            return ColorInfoString(string: string, color: color)
        }
        let prependTo = display["prependTo-\(language)"] as? String
        let dest = display["dest-\(language)"] as! String
        let coSpecialRemark = display["coSpecialRemark-\(language)"] as? String
        let secondLine = display["secondLine-\(language)"] as? String
        let deeplink = display["deeplink"] as! String
        return FavouriteRoute(id: id, groupName: name, routeNumber: routeNumber, co: co, coDisplay: coDisplay, prependTo: prependTo, dest: dest, coSpecialRemark: coSpecialRemark, secondLine: secondLine, deeplink: deeplink)
    }
}
