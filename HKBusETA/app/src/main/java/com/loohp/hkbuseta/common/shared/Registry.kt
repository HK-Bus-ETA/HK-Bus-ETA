            ...
            try {
                ...
                if (stateFlow.value != State.READY) {
                    if (!hasConnection) {
                        ...
                        val textResponse: String = getTextResponseWithPercentageCallback("https://raw.githubusercontent.com/LOOHP/HK-Bus-ETA-WearOS/data/data.json.gz", length, true) { p -> updatePercentageStateFlow.value = p * 0.75f + percentageOffset }
                            ?: throw RuntimeException("Error downloading bus data")
                                ...
                                if (Operator.CTB === Operator.valueOf(bus.optString("co")) && routeNumber == bus.optString("route")) {
                                    ...
                                    if ((stopSeq == (matchingSeq[busDest]?: 0)) && usedRealSeq.computeIfAbsent(busDest) { HashSet() }.add(bus.optInt("eta_seq"))) {
                                        ...
                                        if (eta.isNotEmpty() && !eta.equals("null", ignoreCase = true)) {
                                            ...
                                            ctbEtaEntries.computeIfAbsent(busDest) { ConcurrentMutableSet() }.add(
